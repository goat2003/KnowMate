...
The init script is [shared/sql/init.sql](shared/sql/init.sql). It creates:
...
Generated Markdown files are written to [shared/outputs](shared/outputs). Local development keeps a disabled-network `mock://sample` source for offline smoke tests; production Compose/Kubernetes mounts an explicit crawler config instead of relying on that local mock default.
...
- [shared/config/e2e.config.yaml](shared/config/e2e.config.yaml)
- [shared/config/rss_sources.example.yaml](shared/config/rss_sources.example.yaml)
- [shared/config/user_profile_snapshot.example.json](shared/config/user_profile_snapshot.example.json)
...
Regenerate Python protobuf stubs after editing [shared/proto/agent.proto](shared/proto/agent.proto):
...
Python Agent has a pluggable LLM layer in [python-agent/app/tools/llm_tool.py](python-agent/app/tools/llm_tool.py).
...
Config example in [python-agent/config.yaml](python-agent/config.yaml):
...
Skills live in [python-agent/app/skills](python-agent/app/skills):
...
Python Agent uses the official MCP Python SDK. Client abstractions are in [python-agent/app/mcp](python-agent/app/mcp):
...
Mixed transport configuration in [python-agent/config.yaml](python-agent/config.yaml):
