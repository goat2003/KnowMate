...
> 原文镜像：`shared/config/README.md`

通用配置通过环境变量传递。必要时，每个服务也会保留本地 YAML 文件作为默认配置。

- Backend 默认配置：`goframe-backend/manifest/config/config.yaml`
- Python 默认配置：`python-agent/config.yaml`
- 环境变量示例：`.env.example`
